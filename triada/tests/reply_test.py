...
class TestReply:
    @pytest.mark.asyncio
    async def test_valid_reply(self, mock_vk_client):
        valid_reply_call = await reply_test({
            ...
            "id": 1
        }, called=False, mock_vk_client=mock_vk_client)

        assert valid_reply_call == 1

    @pytest.mark.asyncio
    async def test_invalid_reply(self, mock_vk_client):
        valid_reply_call = await reply_test({
            "text": "АААААААААААААААА БЛЯЯЯЯ",
            "post_id": 123,
            "from_id": 2,
        ...
        assert valid_reply_call == [call('https://api.vk.com/method/wall.deleteComment',
                                         params={'owner_id': -229144827,
                                                 ...
                                                 'v': '5.199'})]
    @pytest.mark.asyncio
    async def test_polemic_reply(self, mock_vk_client):
